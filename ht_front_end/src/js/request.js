...
export const freeRegister = (vm) => {
  const year = vm.form.child_birthday
  vm.form.child_birthday = getBirthday(year)
  http.post(route.register_free, vm.form).then((res) => {
    vm.form.child_birthday = year
    //提交成功后 将状态修改
  ...
  userInfo().then((res) => {
    ...
    freeRegisterInfo().then((register_res) => {
      ...
      if (vm.form.child_birthday == null || vm.form.child_birthday == '') {
        ...
        vm.form.child_birthday = years_of_age(register_res.data.result[0].child_birthday)
      }
    }, (err) => {
      console.log(err);
    })
...
export const getRegister = (vm) => {
  userInfo().then((res) => {
    ...
    freeRegisterInfo().then((register_res) => {
        ...
        getParent().then((parent_res) => {
          if (have_result(parent_res)) {
            ...
            getChildren().then((child_res) => {
              ...
              vm.form.sex = r.sex
              vm.form.child_birthday = years_of_age(r.birthday)
              vm.childNum = 1
            }, (err) => {
              console.log(err);
            });
      ...
      vm.form.schoolid = r.schoolid
      vm.form.child_name = r.child_name
      vm.form.child_birthday = years_of_age(r.child_birthday)
      vm.childNum = 1
    }, (err) => {
      console.log(err);
    })
...
//获取校长信息
export const getAdmin = (vm) => {
  userInfo().then((res) => {
    getSchool(vm);
    if (res_is_success(res)) {
      adminInfo().then((admin_res) => {
        if (have_result(admin_res)) {
          vm.form = admin_res.data[0];
          vm.form.adminid = admin_res.data[0].openid;
        }
      }, (err) => {
        console.log(err);
      });
...
// 校长注册
export const adminRegister = (vm) => {
  userInfo().then((res) => {
    if (res_is_success(res)) {
      http.post(route.admin, vm.form).then((admin_res) => {
        vm.form.adminid = admin_res.data.adminid;
        vm.form.active = admin_res.data.active;
      }, (err) => {
        console.log(err)
      })
    }
