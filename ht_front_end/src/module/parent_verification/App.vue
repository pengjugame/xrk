...
<div :style="{display:formDisplay}" v-if="info.registerid != ''" class="children-div">
  <table height="60px" width="100%;" cellpadding="0" cellspacing="0" class="name">
    <tr>
      <item_td_text note="家长姓名:" />
      <item_td_text ref="ref_cparent_name" v-model="info.parent_name" />
    </tr>
    <tr>
      <item_td_text note="学员名称:" />
      <item_td_text ref="ref_child_name" v-model="info.child_name" />
    </tr>
    <tr>
      <item_td_text note="学员年龄:" />
      <td class="name-td">&nbsp;{{info.birthday | birthdayFilter}}岁</td>
    </tr>
    <tr>
      <item_td_text note="教学点:" style="letter-spacing: 3.7px;" />
      <item_td_text ref="ref_school_name" v-model="info.school_name" />
    </tr>
  </table>
  <div class="eie">
    <a v-bind:href="'tel:' + info.mobile" style="position: relative;top:-4px;color:#4f4f4f">联系电话 {{info.mobile}}</a>
...
export default {
  ...
  filters: {
    birthdayFilter(value) {
      const birthday = tool.years_of_age(value)
      return birthday
    }
  },
  methods: {
