...
exports.add = function(school_admin) {
    return co(function * () {
        ...
        let users_res = yield users.exist(school_admin.openid);
        if (!res_have_result(users_res)) {
            console.error(school_admin.name + '没有关注服务号');
            return Promise.resolve(null);
        }
        //由于教师申请是可以选择性别的  所以需要单独处理；
        users_res.result[0].sex = school_admin.sex
        var new_data = Object.assign(school_admin, users_res.result[0]);
