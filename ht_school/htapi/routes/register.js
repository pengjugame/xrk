...
router.post('/', function(req, res, next) {
    return co(function*() {
        ...
        if (!res_is_success(parent_res)) {
            console.error("错误信息:" + JSON.stringify(parent_res) + " 注册信息:" + JSON.stringify(parent_param));
            res.send(htapi_code(false));
            return Promise.resolve(null);
        }
...
router.delete('/', function(req, res, next) {
    return co(function*() {
        ...
        //给用户移除家长分类tag
        wxapi.moveUserToGroup(param.openid, 0, function(err, data, res) {
            console.log("delete moveUserToGroup: " + param.openid + " err:" + err);
        });
    ...
    return co(function*() {
        ...
        if (res_is_success(teacher_res) && teacher_res.result.length == 1) {
            ...
            if (datas.length == 0) {
                ...
                for (var o in datas) {
                    tmp[o] = datas[o];
                    tmp[o].child_birthday = years_of_age(tmp[o].child_birthday);
                }
                res.send(tmp);
                return Promise.resolve(true);
            }
